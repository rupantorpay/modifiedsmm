...
$apiUrl = "https://payment.rupantorpay.com/";
...
$requestData = [
    ...
    'success_url'  => site_url("addfunds"),
    'cancel_url'    => site_url("addfunds"),
    'webhook_url'   => $paymentURL
];

$host = parse_url($apiUrl,  PHP_URL_HOST);
$postUrl = "https://{$host}/api/payment/checkout";
...
if ($err) {
    ...
    if (isset($result['status']) && isset($result['payment_url'])) {
        ...
        $redirectForm .= '<form method="GET" action=" ' . $paymentUrl . '" name="rupantorpayForm">';
        $redirectForm .= '</form>
        <script type="text/javascript">
        document.rupantorpayForm.submit();
        </script>';
    } else {
